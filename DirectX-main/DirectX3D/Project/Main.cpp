...
ID3D11Device*        device;       //무언가를 만들때 사용 CPU를 다루는 객체 Update
ID3D11DeviceContext* deviceContext;//무언가를 그릴때 사용 GPU를 다루는 객체 Render HDC 와비슷하다

IDXGISwapChain*         swapChain;      //더블버퍼링을 구현하는 개체
ID3D11RenderTargetView* renderTargeView;//백버퍼를 관리하는 객체 view는 GPU가 다루는 객체
                     ...
                     _In_ int       nCmdShow)
{
    ...
    while (true)
    {
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE))//message가 들어오면 true
        {
            ...
            {
                TranslateMessage(&msg);
                DispatchMessage(&msg);//메세지를 전달하다, 올려놓다.
            }
        }
        else
        {
